...
namespace AuroraLib.Compression.Formats.EA
{
    /// <summary>
    ...
    public sealed class RefPack : ICompressionAlgorithm, ILzSettings, IProvidesDecompressedSize
    {
        ...
        {
            return stream.Position + 0x8 < stream.Length && (
                stream.Peek(s =>
                {
                    byte flag = (byte)s.ReadByte();
                    return (flag & 0b0010_1110) == 0 && ((OptionFlags)flag).HasFlag(OptionFlags.Default) && s.ReadByte() == Identifier && s.ReadInt24(Endian.Big) != 0;
                }) || // Version 1 & 3
        ...
        public static void DecompressHeaderless(Stream source, Stream destination, int decomLength)
        {
            ...
            using LzWindows buffer = new LzWindows(destination, lzProperties[0].WindowsBits);
