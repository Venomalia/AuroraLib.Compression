...
namespace AuroraLib.Compression.Formats.Camelot
{
    /// <summary>
    ...
    public sealed class LZ02 : ICompressionAlgorithm, IProvidesDecompressedSize
    {
        ...
        public static bool IsMatchStatic(Stream stream, ReadOnlySpan<char> fileNameAndExtension = default)
            ...
            => (fileNameAndExtension.IsEmpty || PathX.GetExtension(fileNameAndExtension).Contains(_extensions[0].AsSpan(), StringComparison.InvariantCultureIgnoreCase))
                ...
                && stream.Position + 0x8 < stream.Length && stream.Peek(s => Enum.IsDefined(s.Read<DataType>()) && s.ReadUInt24BigEndian() != 0 && (s.ReadByte() & 0x80) == 0);
#else
                && stream.Position + 0x8 < stream.Length && stream.Peek(s => Enum.IsDefined(typeof(DataType), s.Read<DataType>()) && s.ReadUInt24BigEndian() != 0 && (s.ReadByte() & 0x80) == 0);
#endif
